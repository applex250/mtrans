# mtrans - Markdown 学术文献翻译工具

基于智谱 GLM-4.7 的英文学术文献 Markdown 翻译工具，支持智能分段、并发翻译和实时进度显示。
![alt text](image.png)

> **🚀 v2.0 重大更新** - 本次更新引入了全新的过滤机制，大幅提升翻译效率和准确性：
> - ✅ **HTML 表格支持**：新增识别和过滤 HTML 格式表格（`<table>...</table>`）
> - ✅ **过滤优化**：翻译时过滤表格、代码块、图片等特殊元素，节省 50%+ Token 消耗
> - ✅ **智能分段**：基于过滤后的内容进行分段，段落统计更准确
> - ✅ **自动清理**：翻译完成后自动删除临时中间文件
> - ✅ **工作流程重构**：从 5 步优化为 6 步，逻辑更清晰

# 推荐使用方法
https://mineru.net/client 进行pdf转markdown，

然后用该项目进行翻译。

## 重构
把原来使用llm进行分段的程序流改为使用计数程序（达到10000字符）进行分段。
原因：llm分段不稳定，ai对数学计算还不稳定。

## 特性

- ✅ 智能分段：LLM 自动分析文档结构，设计最优分段方案
- ✅ 并发翻译：支持多段并行翻译，速度优先
- ✅ 实时进度：可视化进度条，显示翻译速度
- ✅ 术语一致性：提取专业方向，保持术语翻译一致性
- ✅ 特殊元素过滤与保留：表格、公式、代码块、图片在翻译时过滤，完成后自动还原
- ✅ **HTML 表格支持**：支持识别和处理 HTML 格式表格（`<table>...</table>`）和 Markdown 表格
- ✅ **Token 优化**：过滤特殊元素后翻译，节省 50%+ Token 消耗
- ✅ **自动清理**：翻译完成后自动删除临时中间文件
- 🌐 Web 界面：黑白极客风格的可视化界面，支持拖拽上传和任务队列管理

## 安装

### 1. 克隆项目

```bash
git clone <repository-url>
cd mtrans
```

### 2. 安装依赖

```bash
npm install
```

### 3. 配置环境变量

复制 `.env.example` 为 `.env`：

```bash
cp .env.example .env
```

编辑 `.env` 文件，填写你的智谱 API Key：

```env
ZHIPU_API_KEY=your_api_key_here
ZHIPU_MODEL=glm-4
MAX_CONCURRENCY=5
```

## 使用

### 基本用法

翻译输入文件，自动保存到 `output/` 目录：

```bash
npm test
```

或直接运行：

```bash
node src/index.js input/sample.md
```

### 指定输出文件

```bash
node src/index.js input/paper.md output/translated.md
```

### Web 界面使用

启动 Web 服务器：

```bash
npm run web
```

浏览器访问：`http://localhost:3000`

#### Web 界面功能

- **拖拽上传**：直接拖拽 Markdown 文件到上传区域
- **任务队列**：左侧面板显示所有翻译任务及进度
- **实时日志**：右侧控制台显示详细的翻译步骤和日志
- **自定义输出路径**：可设置统一的输出目录（自动保存到 settings.json）
- **任务管理**：支持暂停、继续、删除翻译任务
- **批量翻译**：支持同时上传多个文档，自动依次执行
- **自动清理**：翻译完成后自动删除 `input/` 目录中的临时文件
- **配置持久化**：设置自动保存到本地，重启后保持配置

#### Web 界面快捷操作

- **暂停任务**：点击任务旁的"暂停"按钮
- **继续任务**：点击任务旁的"继续"按钮
- **删除任务**：点击任务旁的"删除"按钮（仅非执行中任务）
- **清除已完成**：点击底部"清除已完成"按钮清理已完成任务

## 工作流程

1. **解析原始 Markdown**：提取原始文档结构和参考文献
2. **过滤特殊元素**：识别并过滤表格、代码块、图片等特殊元素，保存中间文件到 `temp/` 目录
3. **解析过滤后的 Markdown**：重新解析过滤后的内容，不包含特殊元素
4. **提取专业方向**：分析过滤后的 abstract，生成专业方向描述
5. **设计分段方案**：LLM 根据过滤后的文档结构智能分段
6. **并发翻译并还原**：多段并行翻译过滤后的内容，完成后还原特殊元素并追加参考文献，自动清理临时文件

## 项目结构

```
mtrans/
├── src/
│   ├── parsers/
│   │   └── markdownParser.js    # Markdown 解析器（支持 HTML/Markdown 表格）
│   ├── filters/
│   │   └── elementFilter.js     # 特殊元素过滤器（表格、代码块、图片）
│   ├── llm/
│   │   └── zhipuClient.js       # 智谱 API 客户端
│   ├── translator/
│   │   ├── segmentDesigner.js   # 分段设计器
│   │   └── translator.js        # 翻译器（支持元素还原）
│   ├── web/
│   │   ├── server.js            # Web 服务器
│   │   ├── routes.js            # API 路由（6步工作流程）
│   │   ├── queue.js             # 任务队列管理
│   │   ├── settings.js          # 配置文件管理
│   │   └── public/
│   │       ├── index.html       # 主页面
│   │       ├── style.css        # 极客风格样式
│   │       └── app.js           # 前端逻辑
│   └── index.js                 # 命令行主入口
├── input/                       # 上传文件临时目录（自动清理）
├── output/                      # 翻译结果输出目录
├── temp/                        # 中间文件目录（自动清理，包含过滤后的文件和映射）
├── debug/                       # 调试文件目录
├── .env.example                 # 环境变量模板
├── settings.json                # 用户配置文件（该文件会在用户修改设置时自动生成）
└── package.json
```

## 配置说明

### 环境变量

| 环境变量 | 说明 | 默认值 |
|---------|------|--------|
| `ZHIPU_API_KEY` | 智谱 API Key | 必填 |
| `ZHIPU_MODEL` | 使用的模型 | glm-4 |
| `MAX_CONCURRENCY` | 并发翻译段数 | 5 |
| `WEB_PORT` | Web 服务器端口 | 3000 |

### 配置文件

`settings.json` 该文件会在用户修改设置时,自动生成，包含以下配置：

```json
{
  "outputPath": "output",
  "webPort": 3000
}
```

- `outputPath`: 翻译结果输出路径
- `webPort`: Web 服务器端口

该文件会在 Web 界面修改设置时自动更新。

## 翻译规则

1. 保持学术风格，缩写/术语不翻译
2. **表格**（Markdown 格式 `|...|` 和 HTML 格式 `<table>...</table>`）、**图片**、**公式**、**代码块**在翻译时被过滤，翻译完成后自动还原
3. 保持 markdown 结构
4. 直接输出翻译，无额外文字

## 示例

输入 `input/sample.md` 中的英文文献，输出翻译后的中文版本：

```bash
npm test
```

## 注意事项

- 确保 `.env` 文件中的 API Key 有效
- 翻译过程需要网络连接
- 长文档翻译可能需要较长时间
- 失败的分段会记录到日志并保留原文
- Web 界面上传的文件会自动保存到 `input/` 目录，翻译完成后自动删除
- 翻译过程中的中间文件保存在 `temp/` 目录，翻译完成后自动删除
- `settings.json` 配置文件会在首次运行时自动生成，请勿将其提交到版本控制
- 建议将 `.env` 和 `settings.json` 添加到 `.gitignore` 以保护敏感信息

## v2.0 更新详情

### 新增功能

1. **HTML 表格支持**
   - 支持识别单行 HTML 表格：`<table>...</table>`
   - 支持识别多行 HTML 表格：`<table>` 和 `</table>` 分跨不同行
   - 支持 HTML 表格带属性：`<table class="...">`
   - 保持原有 Markdown 表格识别能力

2. **过滤机制优化**
   - 新增 `elementFilter.js` 模块，统一管理特殊元素过滤
   - 翻译时自动过滤表格、代码块、图片等特殊元素
   - 节省 Token 消耗（通常可节省 50%+）
   - 提高翻译速度

3. **工作流程重构**
   - 从 5 步流程优化为 6 步流程
   - 原始文档仅用于过滤，后续所有操作基于过滤后的内容
   - Abstract 提取、分段设计均基于过滤后的内容，更准确
   - 翻译完成后自动还原特殊元素

4. **中间文件管理**
   - 过滤后的 Markdown 保存到 `temp/` 目录
   - 元素映射关系保存为 JSON 文件
   - 翻译完成后自动清理所有中间文件

### 修改的文件

- `src/filters/elementFilter.js` - 新增特殊元素过滤模块
- `src/parsers/markdownParser.js` - 新增 HTML 表格识别，新增 `parseFilteredMarkdown()` 函数
- `src/translator/translator.js` - 新增元素还原功能
- `src/index.js` - 重构为 6 步工作流程
- `src/web/routes.js` - 同步更新为 6 步工作流程

### 技术改进

- 表格识别正则：`/<table\b/i` 和 `/<\/table>/i`
- 优先级处理：HTML 表格 → Markdown 表格 → 其他特殊元素
- 自动清理：翻译完成后自动删除 `temp/` 目录中的所有中间文件

## 依赖

- `dotenv`: 环境变量管理
- `cli-progress`: 进度条显示
- `p-limit`: 并发控制
- `express`: Web 服务器框架
- `socket.io`: WebSocket 实时通信
- `multer`: 文件上传中间件

## 许可证

MIT
